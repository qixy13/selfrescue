...
![java8大基础数据类型](https://imgconvert.csdnimg.cn/aHR0cHM6Ly9tbWJpei5xcGljLmNuL21tYml6X3BuZy8yQzY1N2t1a0pFSGNuUEU0RmYyWGdmdGt3VGZpYjl1WW95S2lib1RpYTRtRDFBaWNVSWtDblBxcUdNODRUNjNlRDVQMFY1UmpIN3BITk1LTW5SbG9NVHB4M0EvNjQw?x-oss-process=image/format,png)
...
1 通过[Object类](https://so.csdn.net/so/search?q=Object类&spm=1001.2101.3001.7020)的getClass方法来获取
...
在JDK1.7和1.8的实现不同；1.7是数组+链表；1.8是数组+链表+红黑树

1.7

根据key计算index，算出hash值后， index=(n-1)&hash计算index，将键值对插入数组中下标为index的位置；在插入新元素时，如果计算出的index与之前的相同，发生hash冲突，使用头插法用链表的形式插入到index=1的位置，在JDK1.8后改成尾插法。如果index不同则插入到别的index位置。

1.8

每发生一次hash冲突，链表长度都会+1，当链表长度>8时，先调用treeifybin方法，会根据数组的长度来决定是否将链表转变为红黑树，当数组长度>=64的情况下会将链表转为红黑树，如果<64则进行扩容
...
根节点和枝节点上都不会存储数据，只在叶子结点存储数据；

每个叶子节点增加了一个指向相邻叶子结点的指针，形成一个有序链表的结构。
...
保证每次从队列中取出的元素都是权值最小的

通过完全二叉树实现的小顶堆（任意一个非叶子结点的权值，都不大于其左右子节点的权值）

红黑树和最小堆

红黑树插入最差要比较2logN，最小堆最坏情况是logN

红黑树删除不需要比较，只需要不超过3次旋转，而最小堆需要2logN次
...
SqlSessionFactory对象 创建SqlSession的工厂对象 连接数据库

SqlSession对象 以mybatis的方式操作数据库-内部整合JDBC连接
...
Mybatis首先缓存中查询结果集,如果没有则查数据库,如果有则从缓存中取出返回结果集就不走数据库.Mybatis内部缓存使用HashMap, key为HashCode+sqlId+Sql语句.Value为从查询出来映射生成的Java对象Mybatis的二级缓存即查询缓存,它的作用域是一个mapper的namespace,记在同一个那么space中查询sql可以缓存中获取数据.二级缓存可以跨SqlSession的

## 算法

### 排序算法稳定性

只有冒泡、归并、插入排序是稳定的，其他都不稳定